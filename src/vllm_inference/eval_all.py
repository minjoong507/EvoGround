...
def find_dataset_results(dataset_name, split, model_name, exp_id):
    """
    ...
    eval_root = f"./outputs/eval/{model_name}" if exp_id is None else f"./outputs/eval/{exp_id}/"
...
def compute_IoU(pred, gt):
    ...
    # Normalize inverted predictions (start > end) so scoring is symmetric.
    ...
    # Guard against zero-length GT annotations (point moments) to avoid NaN.
    overlap = np.where(union > 0, 1.0 * inter / union, 0.0)
    if not gt_is_list:
        overlap = overlap[:, 0]
    if not pred_is_list:
        overlap = overlap[0]
...
def load_scored_data(data_dir, datasetname):
    ...
    for file in os.listdir(data_dir):
        ...
        for line in open(file_path):
            tmp = json.loads(line)
            cnt += 1
            if datasetname in ["activitynet", "charades", "charades-ood", "tvgbench", "etbenchTG"]:
                score = 0.0
                if None not in tmp["pred"]:
                    score = compute_IoU(tmp["pred"], tmp["target"])
            else:
                if isinstance(tmp["target"], list):
                    tmp["target"] = tmp["target"][0]
...
def eval_rextimeTG(data_dir, datasetname):
    data = []
    pred_qids = []
    cnt = 0
    print(os.listdir(data_dir))
    test_release = json.load(open("video_datasets/ReXTime/data/rextime_test_release.json"))
    ...
    for file in os.listdir(data_dir):
        ...
        for line in open(file_path):
            ...
            record = {
                "qid": tmp["qid"],
                "pred_relevant_windows": [tmp["pred"]],
                "ans": "A"
            }

            pred_qids.append(record["qid"])
            data.append(record)

    missed_gt_list = [itm for itm in test_release if itm["qid"] not in pred_qids]
    print(f"{len(missed_gt_list)} predictions are missing.")

    for missed_itm in missed_gt_list:
        record = {
            "qid": missed_itm['qid'],
            "pred_relevant_windows": [[0, missed_itm['duration']]],
            "ans": "A"
        }
        data.append(record)
...
def calc_score(difficulty_data_dict, datasetname):
    ...
    if datasetname in ["activitynet", "charades", "charades-ood", "tvgbench", "rextimeTG", "etbenchTG"]:
        scores = {}
        ...
        if datasetname in ["etbenchTG"]:
            scores["F1"] = tvg_f1(data)
...
def tvg_f1(samples):
    ...
    out['F1'] = round(sum(recall) / len(recall) * 100, 5)
    return out['F1']
...
def main(args):
    for dataset in args.dataset:
        ...
        if dataset == "rextimeTG":
            load_func = load_rextimeTG
            split = "default"
        if dataset == "etbenchTG":
            load_func = load_etbenchTG
            split = "default"
        if dataset == "timer1":
            load_func = load_timer1
            split = "default"
            ...
            for split in ["multi-choice"]:
                ...
                for data_dir in find_dataset_results(
                    dataset, args.split, args.model_name, args.exp_id
                ):
        ...
        for data_dir in find_dataset_results(dataset, args.split, args.model_name, args.exp_id):
            if dataset == "rextimeTG":
                results_rextime = eval_rextimeTG(data_dir, original_data)
                print(f"{len(results_rextime)} for rextimeTG are saved at {data_dir}")
                save_jsonl(results_rextime, os.path.join(data_dir, "pred.jsonl"))
                continue

            difficulty_data_dict = load_scored_data(data_dir, dataset)
            print(difficulty_data_dict)
            if len(difficulty_data_dict) == 0:
                continue
