...
class BaseDataset(Dataset, ABC):
    ...
    @staticmethod
    def _load_data(dataset_names, split, dataset_path="dataset/config.yaml"):
        ...
        for name in dataset_names:
            ...
            if name == "charades":
                print("Use charades")
                data += load_charades(split=split, cfg=cfg)
            elif name == "activitynet":
                print("Use activitynet")
                data += load_activitynet(split=split, cfg=cfg)
            elif name == "tvgbench":
                print("Use tvgbench")
                data += load_tvgbench(split=split, cfg=cfg)
            elif name == "rextimeTG":
                print("Use rextimeTG")
                data += load_rextimeTG(split=split, cfg=cfg)
            elif name == "etbenchTG":
                print("Use etbenchTG")
                data += load_etbenchTG(split=split, cfg=cfg)
            elif name == "videomme":
                print("Use videomme")
                data += load_videomme(split=split, cfg=cfg)
            elif name == "tempcompass":
                print("Use tempcompass")
                if len(data) > 0 and split == "captioning":
                    assert False
                data += load_tempcompass(split=split, cfg=cfg)
        ...
        if len(data) == 0:
            raise FileNotFoundError("No evaluation data loaded.")
...
class MultipleChoiceQADataset(BaseDataset):
    def __init__(
        ...
        prompt_type="r1",
        use_nothink=False,
        dataset_path="dataset/config.yaml",
        **kwargs,
    ):
        ...
        self.data = self._load_data(dataset_names, split=split, dataset_path=dataset_path)
...
class TemporalGroundingDataset(BaseDataset):
    ...
    def _preprocess(self, itm):
        ...
        tmp = self._load_video_from_cache(itm["video"], ele)
        if tmp is not None:
            video_inputs, video_kwargs = tmp
        else:
            _, video_inputs, utils = process_vision_info_v3(
                messages, return_video_kwargs=True
            )
            video_kwargs = {"fps": utils["fps"]}
            self._save_video_to_cache(itm["video"], ele, (video_inputs, video_kwargs))
        ...
        return {"text": text, "videos": video_inputs, "video_kwargs": video_kwargs}
    ...
    def __getitem__(self, idx):
        ...
        return {
            "inputs": {
                ...
                "multi_modal_data": multi_modal_data,
                "mm_processor_kwargs": inputs.get("video_kwargs", {}),
            },
            "timestamps": self.data[idx]["timestamp"],
            "qid": self.data[idx]["qid"],
            "duration": self.data[idx]["duration"],
